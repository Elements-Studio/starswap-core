module swap_admin::TimelyReleasePool {
    ...
    public fun init<PoolT: store, TokenT: store>(
        ...
        assert!(
            ...
            error::invalid_state(ERROR_LINEAR_RELEASE_EXISTS)
        );
    ...
    public fun withdraw<PoolT: store, TokenT: store>(
        ...
        assert!(now_seconds > pool.begin_time, error::invalid_state(ERROR_EVENT_NOT_START_YET));
    ...
    fun calculate_rounds(now_seconds: u64, latest_release_time: u64, round_interval: u64): u64 {
        ...
        assert!(time_interval >= round_interval, error::invalid_state(ERROR_LINEAR_NOT_READY_YET));
