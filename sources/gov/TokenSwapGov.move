...
module swap_admin::TokenSwapGov {
    ...
    const ERR_INVALID_COIN_AMOUNT: u64 = 204;

    #[test]
    public fun test_all_issued_amount() {
        let total =
    ...
    #[test]
    public fun test_all_linear_treasury() {
    ...
    #[test]
    public fun test_time_linear_withdraw() {
        let precision = STAR::precision();
        let scaling_factor = math64::pow(10, (precision as u64));
        ...
        let farm_can_withdraw_amount = if (elapsed_time >= farm_total_timestamp) {
            ...
            let second_release = math128::mul_div(
                calculate_amount_from_percent(GOV_PERCENT_FARM - GOV_PERCENT_FARM_GENESIS),
    ...
    struct GovTreasuryV2<phantom PoolType> has key, store {
        linear_total: u128,
        linear_treasury: coin::Coin<STAR::STAR>,
        genesis_treasury: coin::Coin<STAR::STAR>,
        locked_start_timestamp: u64,
        locked_total_timestamp: u64,
    }
    ...
    public fun genesis_initialize(account: &signer) {
        ...
        move_to(account, GovTreasury<PoolTypeCommunity> {
            treasury: coin::withdraw<STAR::STAR>(account, (community_total as u64)),
    ...
    public entry fun dispatch<PoolType: store>(
        ...
        let treasury =
            borrow_global_mut<GovTreasuryV2<PoolType>>(signer::address_of(account));
        let disp_token =
            coin::extract<STAR::STAR>(&mut treasury.genesis_treasury, (amount as u64));
        ...
        event::emit(GenesisGovTreasuryWithdrawEvent {
            pool_type: type_name<PoolType>(),
            amount,
    ...
    public entry fun linear_initialize(account: &signer) acquires GovTreasury {
        ...
        let farm_linear = calculate_amount_from_percent(
            GOV_PERCENT_FARM - GOV_PERCENT_FARM_GENESIS
        ) * (scaling_factor as u128);
        ...
        let syrup_linear = calculate_amount_from_percent(
            GOV_PERCENT_SYRUP - GOV_PERCENT_SYRUP_GENESIS
        ) * (scaling_factor as u128);
        STAR::mint(account, syrup_linear);
        move_to(account, GovTreasuryV2<PoolTypeSyrup> {
            linear_total: syrup_linear,
            linear_treasury: coin::withdraw<STAR::STAR>(account, (syrup_linear as u64)),
            genesis_treasury: coin::zero<STAR::STAR>(),
            locked_start_timestamp: GENESIS_TIMESTAMP,
        ...
        let community_linear = calculate_amount_from_percent(
            GOV_PERCENT_COMMUNITY - GOV_PERCENT_COMMUNITY_GENESIS
        ) * (scaling_factor as u128);
        let GovTreasury {
        ...
        move_to(account, GovTreasuryV2<PoolTypeCommunity> {
            ...
            linear_treasury: coin::withdraw<STAR::STAR>(account, (community_linear as u64)),
        ...
        move_to(account, GovTreasuryV2<PoolTypeDeveloperFund> {
            ...
            linear_treasury: coin::withdraw<STAR::STAR>(account, (developerfund_linear as u64)),
            genesis_treasury: coin::zero<STAR::STAR>(),
            locked_start_timestamp: GENESIS_TIMESTAMP,
        ...
        move_to(account, GovTreasuryV2<PoolTypeIDO> {
            linear_total: 0,
            linear_treasury: coin::zero<STAR::STAR>(),
    ...
    fun linear_withdraw<PoolType: store>(account: &signer, to: address, amount: u128) acquires GovTreasuryV2 {
        TokenSwapConfig::assert_global_freeze();
        ...
        let treasury =
            borrow_global_mut<GovTreasuryV2<PoolType>>(signer::address_of(account));
        coin::deposit<STAR::STAR>(
            to,
            coin::extract<STAR::STAR>(&mut treasury.linear_treasury, (amount as u64))
        );

        // let treasury_event = borrow_global_mut<GovTreasuryEvent<PoolType>>(signer::address_of(account));
        event::emit(LinearGovTreasuryWithdrawEvent {
            pool_type: type_name<PoolType>(),
            amount,
    ...
    fun linear_withdraw_farm_syrup<PoolType: store>(
        account: &signer
    ): coin::Coin<STAR::STAR> acquires GovTreasuryV2 {
        TokenSwapConfig::assert_global_freeze();
        ...
        let disp_token = coin::extract<STAR::STAR>(
            &mut treasury.linear_treasury,
            (can_withdraw_amount as u64)
        );
        // let treasury_event = borrow_global_mut<GovTreasuryEvent<PoolType>>(STAR::token_address());

        event::emit(
            LinearGovTreasuryWithdrawEvent {
                pool_type: type_name<PoolType>(),
    ...
    public fun linear_withdraw_farm(account: &signer, _amount: u128) acquires GovTreasuryV2 {
    ...
    public fun get_can_withdraw_of_linear_treasury<PoolType: store>(): u128 acquires GovTreasuryV2 {
        ...
        if (coin::value<STAR::STAR>(&treasury.linear_treasury) < ((treasury.linear_total - amount) as u64)) {
            0
        } else {
    ...
    public fun test_get_can_withdraw_of_linear_treasury() {
        let precision = STAR::precision();
        let scaling_factor = math64::pow(10, (precision as u64));
        let total = calculate_amount_from_percent(
            GOV_PERCENT_FARM - GOV_PERCENT_FARM_GENESIS
        ) * (scaling_factor as u128) - 1263196461020203;
    ...
    public fun get_circulating_supply(): u128 acquires GovTreasuryV2 {
        let total = option::destroy_some(coin::supply<STAR::STAR>());
    ...
    public fun aptos_genesis_burn(
        signer: &signer,
        farm_burn_amount: u128,
        sryup_burn_amount: u128
    ) acquires GovTreasuryV2 {
        STAR::assert_genesis_address(signer);
        let precision = STAR::precision();
        let scaling_factor = math64::pow(10, (precision as u64));
        ...
        assert!(
            farm_treasury.linear_total == calculate_amount_from_percent(
                GOV_PERCENT_FARM - GOV_PERCENT_FARM_GENESIS
            ) * (scaling_factor as u128),
            100
        );
        let farm_treasury_amount = coin::value(&farm_treasury.linear_treasury);
        let farm_treasury_burn = coin::extract(&mut farm_treasury.linear_treasury, (farm_burn_amount as u64));
        assert!(
            farm_treasury_amount == coin::value(&farm_treasury_burn),
            error::invalid_state(ERR_INVALID_COIN_AMOUNT)
        );
        coin::deposit(signer::address_of(signer), farm_treasury_burn);
        STAR::burn(signer, farm_burn_amount);

        event::emit(LinearGovTreasuryMultiBurnEvent {
            amount: farm_burn_amount,
            chain: b"Aptos_Multi_Genesis_Farm",
            treasury_type: type_name<PoolTypeFarmPool>(),
        });

        farm_treasury.linear_total = farm_treasury.linear_total - ((farm_treasury_amount as u128) / 3) ;

        let syrup_treasury =
            borrow_global_mut<GovTreasuryV2<PoolTypeSyrup>>(STAR::token_address());
        assert!(
            syrup_treasury.linear_total == calculate_amount_from_percent(
                GOV_PERCENT_SYRUP - GOV_PERCENT_SYRUP_GENESIS
            ) * (scaling_factor as u128),
            100
        );
        let syrup_treasury_amount = coin::value(&syrup_treasury.linear_treasury);
        let syrup_treasury_burn = coin::extract(&mut syrup_treasury.linear_treasury, (sryup_burn_amount as u64));
        STAR::burn_coin(signer, syrup_treasury_burn);
        syrup_treasury.linear_total = syrup_treasury.linear_total - ((syrup_treasury_amount as u128) / 3) ;

        event::emit(LinearGovTreasuryMultiBurnEvent {
            amount: sryup_burn_amount,
            chain: b"Aptos_Multi_Genesis_Syrup",
            treasury_type: type_name<PoolTypeSyrup>()
        });
    }

    public fun aptos_genesis_burn_community(
        signer: &signer,
        community_burn_amount: u128
    ) acquires GovTreasuryV2 {
        STAR::assert_genesis_address(signer);
        ...
        // let event = &mut borrow_global_mut<GovTreasuryMultiChainEvent>(
        //     address_of(signer)
        // ).burn_linearGovTreasury_event_handler;

        let community_treasury =
            borrow_global_mut<GovTreasuryV2<PoolTypeCommunity>>(STAR::token_address());
        assert!(
            community_treasury.linear_total == calculate_amount_from_percent(
                GOV_PERCENT_COMMUNITY - GOV_PERCENT_COMMUNITY_GENESIS
            ) * (scaling_factor as u128),
            100
        );
        let community_treasury_amount = coin::value(&community_treasury.linear_treasury);
        let community_treasury_burn = coin::extract(
            &mut community_treasury.linear_treasury,
            (community_burn_amount as u64)
        );
        event::emit(LinearGovTreasuryMultiBurnEvent {
            amount: (community_treasury_amount as u128),
            chain: b"Aptos_Multi_Genesis_Communtiy",
            treasury_type: type_name<PoolTypeFarmPool>()
        });
        STAR::burn_coin(signer, community_treasury_burn);
