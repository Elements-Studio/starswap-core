...
module swap_admin::TokenSwapConfig {
    ...
    public fun get_swap_fee_operation_rate(): (u64, u64) {
        if (on_chain_config::config_exist_by_address<SwapFeeOperationConfig>(admin_address())) {
            let conf = on_chain_config::get_by_address<SwapFeeOperationConfig>(admin_address());
    ...
    public fun get_fee_auto_convert_switch(): bool {
        if (on_chain_config::config_exist_by_address<SwapFeeSwitchConfig>(admin_address())) {
            let conf = on_chain_config::get_by_address<SwapFeeSwitchConfig>(admin_address());
    ...
    public fun set_swap_fee_operation_rate(signer: &signer, num: u64, denum: u64) {
        ...
        if (on_chain_config::config_exist_by_address<SwapFeeOperationConfig>(admin_address())) {
            on_chain_config::set<SwapFeeOperationConfig>(signer, config);
        } else {
            on_chain_config::publish_new_config<SwapFeeOperationConfig>(signer, config);
        }
    }
    ...
    public fun set_swap_fee_operation_rate_v2<X,
        ...
        if (on_chain_config::config_exist_by_address<SwapFeeOperationConfigV2<X, Y>>(admin_address())) {
            on_chain_config::set<SwapFeeOperationConfigV2<X, Y>>(signer, config);
        } else {
            on_chain_config::publish_new_config<SwapFeeOperationConfigV2<X, Y>>(signer, config);
        }
    }
    ...
    public fun set_poundage_rate<X,
        ...
        if (on_chain_config::config_exist_by_address<SwapFeePoundageConfig<X, Y>>(admin_address())) {
            on_chain_config::set<SwapFeePoundageConfig<X, Y>>(signer, config);
        } else {
            on_chain_config::publish_new_config<SwapFeePoundageConfig<X, Y>>(signer, config);
        }
    }
    ...
    public fun put_stepwise_multiplier(
        ...
        if (on_chain_config::config_exist_by_address<SwapStepwiseMultiplierConfig>(admin_address())) {
            let conf = on_chain_config::get_by_address<SwapStepwiseMultiplierConfig>(admin_address());
            ...
            // Reset to config
            on_chain_config::set<SwapStepwiseMultiplierConfig>(signer, SwapStepwiseMultiplierConfig {
                list: *&conf.list
            });
        } else {
            ...
            on_chain_config::publish_new_config<SwapStepwiseMultiplierConfig>(signer, SwapStepwiseMultiplierConfig {
                list: step_mutiplier,
            });
        }
    }
    ...
    public fun set_fee_auto_convert_switch(signer: &signer, auto_convert_switch: bool) {
        ...
        if (on_chain_config::config_exist_by_address<SwapFeeSwitchConfig>(admin_address())) {
            on_chain_config::set<SwapFeeSwitchConfig>(signer, config);
        } else {
            on_chain_config::publish_new_config<SwapFeeSwitchConfig>(signer, config);
        }
    }
    ...
    public fun set_global_freeze_switch(signer: &signer, freeze_switch: bool) {
        ...
        if (on_chain_config::config_exist_by_address<SwapGlobalFreezeSwitch>(admin_address())) {
            on_chain_config::set<SwapGlobalFreezeSwitch>(signer, config);
        } else {
            on_chain_config::publish_new_config<SwapGlobalFreezeSwitch>(signer, config);
        }
    }
    ...
    public fun get_global_freeze_switch(): bool {
        if (on_chain_config::config_exist_by_address<SwapGlobalFreezeSwitch>(admin_address())) {
            let conf = on_chain_config::get_by_address<SwapGlobalFreezeSwitch>(admin_address());
            conf.freeze_switch
        } else {
    ...
    public fun set_white_list_boost_switch(signer: &signer, white_list_switch: bool, white_list_pubkey: vector<u8>) {
        ...
        if (on_chain_config::config_exist_by_address<WhiteListBoostSwitch>(admin_address())) {
            on_chain_config::set<WhiteListBoostSwitch>(signer, config);
        } else {
            on_chain_config::publish_new_config<WhiteListBoostSwitch>(signer, config);
        }
    }
    ...
    public fun get_white_list_boost_switch(): (bool, vector<u8>) {
        if (on_chain_config::config_exist_by_address<WhiteListBoostSwitch>(admin_address())) {
            let conf = on_chain_config::get_by_address<WhiteListBoostSwitch>(admin_address());
