...
module swap_admin::TokenSwapLibrary {
    use std::error;
    ...
    public fun quote(amount_x: u128, reserve_x: u128, reserve_y: u128): u128 {
        assert!(amount_x > 0, error::invalid_state(ERROR_ROUTER_PARAMETER_INVALID));
        assert!(reserve_x > 0 && reserve_y > 0, error::invalid_state(ERROR_ROUTER_PARAMETER_INVALID));
    ...
    public fun get_amount_in(amount_out: u128,
        ...
        assert!(reserve_in > 0 && reserve_out > 0, error::invalid_state(ERROR_ROUTER_PARAMETER_INVALID));
        assert!(fee_denumerator > 0 && fee_numerator > 0, error::invalid_state(ERROR_SWAP_FEE_ALGORITHM_INVALID));
        assert!(fee_denumerator > fee_numerator, error::invalid_state(ERROR_SWAP_FEE_ALGORITHM_INVALID));
        assert!(reserve_out > amount_out, error::invalid_state(ERROR_SWAP_FEE_ALGORITHM_INVALID));
...
//        r + 1
    ...
    public fun get_amount_out(amount_in: u128,
                              ...
                              fee_numerator: u64,
                              fee_denumerator: u64): u128 {
        assert!(amount_in > 0, error::invalid_state(ERROR_ROUTER_PARAMETER_INVALID));
        assert!(reserve_in > 0 && reserve_out > 0, error::invalid_state(ERROR_ROUTER_PARAMETER_INVALID));
...
//        r
    ...
    public fun get_amount_in_without_fee(amount_out: u128, reserve_in: u128, reserve_out: u128): u128 {
        assert!(amount_out > 0, error::invalid_state(ERROR_ROUTER_PARAMETER_INVALID));
        assert!(reserve_in > 0 && reserve_out > 0, error::invalid_state(ERROR_ROUTER_PARAMETER_INVALID));
