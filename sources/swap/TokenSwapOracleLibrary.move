...
module swap_admin::TokenSwapOracleLibrary {
    ...
    public fun current_cumulative_prices<X, Y>(): (u128, u128, u64) {
        ...
        let price_y_cumulative_decode =
            FixedPoint128::decode(FixedPoint128::encode_u256(price_y_cumulative, false));
    ...
    public fun current_cumulative_prices_v2<X, Y>(): (u256, u256, u64) {
        ...
        if (last_block_timestamp != block_timestamp) {
            ...
            if (x_reserve != 0 && y_reserve != 0) {
                ...
                // counterfactual
                let new_price_x_cumulative = FixedPoint128::to_u256(
                    FixedPoint128::div(FixedPoint128::encode(y_reserve), x_reserve)
                ) * (time_elapsed as u256);
                let new_price_y_cumulative =
                    FixedPoint128::to_u256(
                        FixedPoint128::div(FixedPoint128::encode(x_reserve), y_reserve)
                    ) * (time_elapsed as u256);
                price_x_cumulative = price_x_cumulative * new_price_x_cumulative;
                price_y_cumulative = price_y_cumulative * new_price_y_cumulative;
            };
        };
