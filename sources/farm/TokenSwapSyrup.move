...
module swap_admin::TokenSwapSyrup {
    ...
    struct SyrupExtInfoV2<phantom TokenT> has key, store {
        multiplier_pool_cap: TokenSwapSyrupMultiplierPool::PoolCapability<PoolTypeSyrup, coin::Coin<TokenT>>,
        alloc_point: u128,
    }
    ...
    //     //     AddPoolEvent{
    //     //         token_code: coin::Cointoken_code<TokenT>(),
    ...
    public fun add_pool_v2<TokenT: store>(
        ...
        let multiplier_pool_cap =
            TokenSwapSyrupMultiplierPool::initialize<PoolTypeSyrup, coin::Coin<TokenT>>(signer);
    ...
    public fun update_token_pool_index<TokenT>(signer: &signer) acquires Syrup {
        ...
        YieldFarming::update_pool_index<PoolTypeSyrup, STAR::STAR, coin::Coin<TokenT>>(
            &syrup.param_cap,
            broker_addr(),
        );
    }
    ...
    //     // syrup.release_per_second = release_per_second;
    //     // YieldFarmingV3::update_pool<PoolTypeSyrup, STAR::STAR, coin::Coin<TokenT>>(
    //     //     &syrup.param_cap,
    //     //     broker_addr(),
    ...
    public fun put_stepwise_multiplier<TokenT: store>(
        ...
        TokenSwapSyrupMultiplierPool::add_pool<PoolTypeSyrup, coin::Coin<TokenT>>(
    ...
    //     });
    //
    //     coin::Coinburn<TokenT>(account, YieldFarming::withdraw<PoolType, TokenT>(account, amount));
    // }

    /// withdraw Toke the pool
    public fun withdraw<PoolType: store, TokenT: copy + drop + store>(
        account: &signer,
        amount: u128):coin::Coin<TokenT> {
        STAR::assert_genesis_address(account);
    ...
    public fun stake<TokenT: store>(
        ...
        TokenSwapSyrupMultiplierPool::add_amount<PoolTypeSyrup, coin::Coin<TokenT>>(
            broker_addr,
        ...
        let (harvest_cap, id) = YieldFarming::stake_v2<PoolTypeSyrup, STAR::STAR, coin::Coin<TokenT>>(
            signer,
            broker_addr,
            stake_token,
    ...
    public fun unstake<TokenT: store>(signer: &signer, id: u64): (
        ...
        let (
            unstaken_token,
            reward_token
        ) = YieldFarming::unstake<PoolTypeSyrup, STAR::STAR, coin::Coin<TokenT>>(
            signer,
            broker_addr,
            harvest_cap
        );
        ...
        TokenSwapSyrupMultiplierPool::remove_amount<PoolTypeSyrup, coin::Coin<TokenT>>(
            broker_addr,
            &syrup_ext.multiplier_pool_cap,
            &key,
            token_amount,
    ...
    public fun query_multiplier_pool_info<TokenT: store>(pledge_time: u64): (u64, u128, u128) {
        TokenSwapSyrupMultiplierPool::query_pool_by_key<PoolTypeSyrup, coin::Coin<TokenT>>(
            broker_addr(),
    ...
    public fun pledge_time_to_mulitplier<TokenT>(pledge_time_sec: u64): u64 {
        ...
        if (TokenSwapSyrupMultiplierPool::has<PoolTypeSyrup, coin::Coin<TokenT>>(broker_addr(), &key)) {
    ...
    public fun adjust_total_amount<TokenT: store>(
        ...
        YieldFarming::adjust_total_amount<PoolTypeSyrup, coin::Coin<TokenT>>(
            &syrup.param_cap,
            broker_addr(),
            total_amount,
            total_weight
    ...
    public fun set_multiplier_pool_amount<TokenT: store>(
        ...
        TokenSwapSyrupMultiplierPool::set_pool_amount<PoolTypeSyrup, coin::Coin<TokenT>>(
            broker_addr(),
            &ext_v2.multiplier_pool_cap,
            &pledge_time_to_key(pledge_time),
            amount,
