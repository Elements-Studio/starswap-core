...
module swap_admin::TokenSwapVestarMinter {
    ...
    public fun init(signer: &signer): (MintCapability, TreasuryCapability) {
        assert!(signer::address_of(signer) == @swap_admin, error::invalid_state(ERROR_NOT_ADMIN));
    ...
    acquires VestarOwnerCapability, Treasury, MintRecordListT, VestarEventHandler, MintRecordList {
        let broker = type_address<VESTAR::VESTAR>();
    ...
    fun test_add_new_record_for_compatibility(signer: signer) acquires MintRecordListT, MintRecordList {
        ...
        add_to_record<starcoin_coin::STC>(&signer, 2, 200, 200, 200);
        ...
        assert!(value_of_id_by_token<starcoin_coin::STC>(user_addr, 2) == 200, 10005);
    }
