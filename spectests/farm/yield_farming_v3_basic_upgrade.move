...
//# publish
module SwapAdmin::YieldFarmingWarpper {
    ...
    struct STARWarpper has copy, drop, store {}
    ...
    public fun initialize(signer: &signer, treasury: Token::Token<STARWarpper>) {
        YieldFarming::initialize<PoolType_A, STARWarpper>(signer, treasury);
    ...
    public fun update_pool(signer: &signer,  alloc_point: u128, last_alloc_point: u128) acquires GovModfiyParamCapability {
        ...
        YieldFarming::update_pool<PoolType_A, STARWarpper, AssetType_A>(
            &cap.cap,
            @SwapAdmin,
            alloc_point,
            last_alloc_point);
    ...
    acquires GovModfiyParamCapability, StakeCapbabilityList {
        ...
        let (harvest_cap, stake_id) = YieldFarming::stake_v2<PoolType_A, STARWarpper, AssetType_A>(
            signer,
            @SwapAdmin,
    ...
    public fun unstake(signer: &signer, id: u64): (u128, u128) acquires StakeCapbabilityList {
        ...
        let (asset, token) = YieldFarming::unstake<PoolType_A, STARWarpper, AssetType_A>(signer, @SwapAdmin, cap);
        let token_val = Token::value<STARWarpper>(&token);
        Account::deposit<STARWarpper>(Signer::address_of(signer), token);
        (asset.value, token_val)
    }

    public fun harvest(signer: &signer, id: u64): Token::Token<STARWarpper> acquires StakeCapbabilityList {
        assert!(id > 0, 10000);
        let cap_list = borrow_global_mut<StakeCapbabilityList>(Signer::address_of(signer));
        let cap = Vector::borrow(&cap_list.items, id - 1);
        YieldFarming::harvest<PoolType_A, STARWarpper, AssetType_A>(Signer::address_of(signer), @SwapAdmin, 0, cap)
    }

    public fun query_expect_gain(user_addr: address, id: u64): u128 acquires StakeCapbabilityList {
        let cap_list = borrow_global_mut<StakeCapbabilityList>(user_addr);
        let cap = Vector::borrow(&cap_list.items, id - 1);
        YieldFarming::query_expect_gain<PoolType_A, STARWarpper, AssetType_A>(user_addr, @SwapAdmin, cap)
    }
...
script {
    ...
    use SwapAdmin::YieldFarmingWarpper::{STARWarpper};
    use SwapAdmin::CommonHelper;

    /// Initial reward token, registered and mint it
    fun accept_and_deposit(signer: signer) {
        // Resister and mint STARWarpper
        Token::register_token<STARWarpper>(&signer, 9u8);
        Account::do_accept_token<STARWarpper>(&signer);

        let star_token = Token::mint<STARWarpper>(&signer, CommonHelper::pow_amount<STARWarpper>(100000000));
...
script {
    use StarcoinFramework::Account;
    use SwapAdmin::YieldFarmingWarpper::{STARWarpper, Self};
    use SwapAdmin::CommonHelper;

    /// Inital token into yield farming treasury
    fun init_token_into_treasury(account: signer) {
        let star_amount = CommonHelper::pow_amount<STARWarpper>(10000);

        let treasury = Account::withdraw(&account, star_amount);
        YieldFarmingWarpper::initialize(&account, treasury);
    }
}
// check: EXECUTED
...
script {
    ...
    fun stake_token_to_pool(signer: signer) {
        // Alice accept STARWarpper
        Account::do_accept_token<STARWarpper>(&signer);

        //when pool type = farm, boost factor == 1.0, then weight_factor = 100
        let asset_amount = CommonHelper::pow_amount<STARWarpper>(1);
        let asset_weight = asset_amount * 1;
        let stake_id = YieldFarmingWarpper::stake_v2(&signer, asset_amount, asset_weight, 100, 0);
...
script {
    ...
    use SwapAdmin::YieldFarmingWarpper::{Self};
...
script {
    ...
    fun alice_unstake_afeter_3_seconds(signer: signer) {
        ...
        let lp_release_per_second = 800000000;
        assert!(asset_val == CommonHelper::pow_amount<STARWarpper>(1), 1010);
        assert!(token_val == (lp_release_per_second * 3), 1011);

        let (_, asset_total_amount, _, _) = YieldFarmingWarpper::query_pool_info_v2();
...
script {
    ...
    fun bob_stake_1(signer: signer) {
        ...
        let asset_amount = CommonHelper::pow_amount<STARWarpper>(1);
        let asset_weight = asset_amount * 1;
        let stake_id = YieldFarmingWarpper::stake_v2(&signer, asset_amount, asset_weight, 100, 60);
...
script {
    ...
    fun bob_harvest_deadline60_after4sec_check_abort(signer: signer) {
        ...
        Account::deposit<YieldFarmingWarpper::STARWarpper>(Signer::address_of(&signer), token);
    }
}
...
script {
    ...
    fun bob_stake_2(signer: signer) {
        ...
        let stake_id = YieldFarmingWarpper::stake_v2(&signer, asset_amount, asset_weight, 150, 0);
...
script {
    ...
    fun bob_stake_3(signer: signer) {
        ...
        let stake_id = YieldFarmingWarpper::stake_v2(&signer, asset_amount, asset_weight, 200, 0);
...
script {
    ...
    use SwapAdmin::YieldFarmingWarpper;
    ...
    fun bob_harvest_2(signer: signer) {
        ...
        Account::deposit<YieldFarmingWarpper::STARWarpper>(user_addr, token2);
        Debug::print(&amount2);
